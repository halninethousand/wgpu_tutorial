...
impl PipelineBuilder {
    ...
    pub fn build_pipeline(&self, device: &wgpu::Device) -> wgpu::RenderPipeline {
        ...
        let source_code = fs::read_to_string(filepath).expect("Cant read from source code!");
        ...
        let pipiline_layout_descriptor = wgpu::PipelineLayoutDescriptor {
            label: Some("Render Pipeline Layout"),
            bind_group_layouts: &[],
            push_constant_ranges: &[],
        };
        let pipeline_layout = device.create_pipeline_layout(&pipiline_layout_descriptor);
